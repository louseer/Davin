<template>
  <div class="tabbar">
    <div class="tabtop">
      <div class="tabbtn">
        <b
          v-for="(tab,tindex) in ntabs"
          @click="changetab(tab)"
          :class="tab.active ? 'tabon' : ''"
        >{{tab.name}}</b>
      </div>
      <!-- switch  -->
    ...
    <div class="tabcontent" ref="tabcontent">
      <template v-for="(tab,index) in ntabs">
        <div v-show="tab.active">
          <slot :name="`tab${tab.id}`" :height="height"></slot>
        </div>
      </template>
    </div>
  </div>
...
export default {
  ...
  mounted() {
    this.height = '' + this.$refs.tabcontent.clientHeight
    window.onresize=()=>{
       this.height = '' + this.$refs.tabcontent.clientHeight
